...
APP_VERSION = "v9 · tickets, reality & release plan · 2026-06-26"
...
TICKET_COLUMNS = ["Key", "Summary", "Category", "Team", "Client",
                  "WorkingDays", "Month", "Status"]
RELEASE_COLUMNS = ["Release", "TargetMonth", "Status", "Scope"]

# Ticket categories map to the "reality" side of plan-vs-reality. The first
# four align with the project-allocation categories (so allocated capacity has
# a counterpart); "Admin and Support" and "S&S" are overhead categories with
# no allocated-capacity plan yet.
TICKET_CATEGORIES = ["Roadmap", "C&I", "Support", "IMS Modernization",
                     "Admin and Support", "S&S"]
TICKET_STATUSES = ["To Do", "In Progress", "In Review", "Done", "Blocked"]
...
TICKET_SEED = [
    {"Key": "SWE-101", "Summary": "New billing API", "Category": "Roadmap",
     "Team": "SWE", "Client": "Acme", "WorkingDays": 12, "Month": "August",
     "Status": "In Progress"},
    {"Key": "SWA-58", "Summary": "Tenant migration tooling", "Category": "C&I",
     "Team": "SWA", "Client": "Globex", "WorkingDays": 8, "Month": "August",
     "Status": "To Do"},
    {"Key": "CLD-9", "Summary": "Cluster failover incident",
     "Category": "Support", "Team": "Cloud", "Client": "Acme",
     "WorkingDays": 3, "Month": "September", "Status": "Done"},
    {"Key": "IMS-4", "Summary": "Legacy IMS decommission",
     "Category": "IMS Modernization", "Team": "SWE", "Client": "Internal",
     "WorkingDays": 15, "Month": "September", "Status": "To Do"},
]
RELEASE_SEED = [
    {"Release": "2026.08 – Billing GA", "TargetMonth": "August",
     "Status": "In Progress", "Scope": "Billing API + migration tooling"},
    {"Release": "2026.09 – Platform 2.0", "TargetMonth": "September",
     "Status": "Planned", "Scope": "IMS decommission, failover hardening"},
]
...
def _coerce_tickets_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in TICKET_COLUMNS:
        if col not in df.columns:
            df[col] = 0.0 if col == "WorkingDays" else ""
    df = df[TICKET_COLUMNS]
    df["WorkingDays"] = pd.to_numeric(df["WorkingDays"], errors="coerce").fillna(0.0)
    for c in ("Key", "Summary", "Category", "Team", "Client", "Month", "Status"):
        df[c] = df[c].astype(str).replace({"nan": "", "None": ""}).str.strip()
    # Keep rows that have any identifying content.
    df = df[(df["Key"] != "") | (df["Summary"] != "") | (df["WorkingDays"] > 0)]
    return df.reset_index(drop=True)
...
def ticket_reality(tickets: pd.DataFrame, month_labels) -> pd.DataFrame:
    """Sum ticket working-days by Category x Month (the 'reality')."""
    idx = TICKET_CATEGORIES
    out = pd.DataFrame(0.0, index=idx, columns=list(month_labels))
    if tickets.empty:
        return out
    g = tickets.groupby(["Category", "Month"])["WorkingDays"].sum()
    for (cat, month), days in g.items():
        if cat in out.index and month in out.columns:
            out.loc[cat, month] += float(days)
    return out
...
with tab_tickets:
    ...
    st.caption("Log delivery work: each ticket's category, the team/client, "
               "how many working days it takes, and the month it lands in. "
               "These roll up into the 'reality' compared against allocated "
               "capacity below.")

    team_opts = sorted(set(KNOWN_TEAMS) |
                       {t for t in team_df["Team"].unique() if t})
    tickets_edit = st.data_editor(
        base_tickets, num_rows="dynamic", use_container_width=True,
        key="tickets_editor",
        column_config={
            "Key": st.column_config.TextColumn("Ticket", help="e.g. SWE-123"),
            "Summary": st.column_config.TextColumn("Summary", width="large"),
            "Category": st.column_config.SelectboxColumn(
                "Category", options=TICKET_CATEGORIES, required=True),
            "Team": st.column_config.SelectboxColumn("Team", options=team_opts),
            "Client": st.column_config.TextColumn("Client"),
            "WorkingDays": st.column_config.NumberColumn(
                "Working days", min_value=0.0, step=0.5, format="%.1f"),
            "Month": st.column_config.SelectboxColumn(
                "Month", options=MONTH_LABELS),
            "Status": st.column_config.SelectboxColumn(
                "Status", options=TICKET_STATUSES),
        })
    ...
    t1.metric("Tickets", len(tickets_df))
    t2.metric("Total ticket days", f"{tickets_df['WorkingDays'].sum():,.1f}")
    open_days = tickets_df.loc[tickets_df["Status"] != "Done",
                               "WorkingDays"].sum()
    t3.metric("Open (not Done) days", f"{open_days:,.1f}")

    # ---- Allocated capacity vs reality, per category ----
    st.subheader("Allocated capacity vs reality — per category")
    st.caption("Allocated = capacity assigned to each project via the team "
               "split. Reality = ticket working-days. Δ = allocated − reality "
               "(positive = allocated capacity not yet consumed; negative = "
               "tickets exceed the allocated capacity).")
    alloc_t = project_allocation(matrix, split_df, MONTH_LABELS)
    allocated = (alloc_t.groupby("Project")[MONTH_LABELS].sum()
                 if not alloc_t.empty else
                 pd.DataFrame(0.0, index=PROJECTS, columns=MONTH_LABELS))
    allocated = allocated.reindex(TICKET_CATEGORIES).fillna(0.0)
    reality = ticket_reality(tickets_df, MONTH_LABELS)

    for cat in TICKET_CATEGORIES:
        a = allocated.loc[cat]
        r = reality.loc[cat]
        comp = pd.DataFrame({"Allocated": a, "Reality (tickets)": r,
                             "Δ": a - r}).T
        st.markdown(f"**{cat}**"
                    + ("  — no allocated-capacity plan (overhead category)"
                       if cat in ("Admin and Support", "S&S") else ""))
        st.dataframe(
            comp, use_container_width=True,
            column_config={m: st.column_config.NumberColumn(m, format="%.1f")
                           for m in MONTH_LABELS})

    st.subheader("Allocated vs reality (period total)")
    summ = pd.DataFrame({
        "Allocated": allocated.sum(axis=1),
        "Reality": reality.sum(axis=1)}).reindex(TICKET_CATEGORIES)
    st.bar_chart(summ, color=[BRAND_BLUE, BRAND_ORANGE])

    # ---- Reality by client (seed for 'client budget vs reality') ----
    if not tickets_df.empty and tickets_df["Client"].str.strip().any():
        st.subheader("Delivered days by client (reality)")
        st.caption("Reality side of 'client budget vs reality'. Add agreed "
                   "client budgets later to complete the comparison.")
        by_client = (tickets_df.assign(
            Category=lambda d: d["Category"].where(
                d["Category"].isin(["Roadmap", "C&I"]), "Other"))
            .pivot_table(index="Client", columns="Category",
                         values="WorkingDays", aggfunc="sum", fill_value=0.0))
        st.dataframe(by_client, use_container_width=True)

    # ---- Release plan ----
    st.divider()
    st.subheader("Release plan")
    st.caption("Starter release schedule — tell me what else you need here "
               "(linked tickets, scope readiness, go/no-go) and I'll extend it.")
    release_edit = st.data_editor(
        ...
        column_config={
            ...
            "TargetMonth": st.column_config.SelectboxColumn(
                "Target month", options=MONTH_LABELS),
            "Status": st.column_config.SelectboxColumn(
                "Status", options=RELEASE_STATUSES),
            "Scope": st.column_config.TextColumn("Scope / notes", width="large"),
        })
