...
APP_VERSION = "v11 · release-driven capacity check · 2026-06-26"

# Forecast months. (Excel column letter, label, calendar month number).
# Month range is computed dynamically from the forecast year (see
# months_for_year). For the current year it shows the remaining months; for a
# future year it shows the full Jan–Dec. The static list below is just a safe
# default used until the sidebar reassigns these globals at runtime.
...
def months_for_year(year: int, today: Optional["dt.date"] = None) -> list:
    """Return [(excel_col_letter, label, month_number), ...] for a forecast year.

    - Future year  -> all 12 months (January–December).
    - Current year -> the remaining months, i.e. from the month AFTER the
      current (in-progress) month to December. e.g. in June 2026 -> Jul–Dec.
    ...
    if year > today.year:
        ...
        start = today.month + 1          # remaining (not-yet-started) months
    else:
        start = 1
    nums = [m for m in range(1, 13) if m >= start]
    if not nums:                         # e.g. December of the current year
        nums = [12]
    # Excel columns start at I (9) regardless of how many months there are.
    return [(_gcl(9 + i), MONTH_NAMES[m - 1], m) for i, m in enumerate(nums)]


MONTHS = months_for_year(2026, dt.date(2026, 6, 1))   # safe default (Jul–Dec)
...
# Project categories each team's capacity is split across.
# C&I = Customization & Integration; IMS = IMS Modernization.
PROJECTS = ["Roadmap", "C&I", "Support", "IMS Modernization"]

# Worksheets / local files for the project configuration.
SPLIT_WS = "Splits"
AGREED_WS = "Agreed"
LOCAL_SPLIT_FILE = os.path.join(LOCAL_DATA_DIR, "splits.csv")
LOCAL_AGREED_FILE = os.path.join(LOCAL_DATA_DIR, "agreed.csv")

SPLIT_COLUMNS = ["Team"] + PROJECTS
AGREED_COLUMNS = ["Category", "Month", "Agreed"]

# Default per-team split (%) from the project-division screenshot. IMS
# Modernization defaults to 0% (no team allocation yet); QA is a blended
# placeholder (its data splits into QA-Development / QA-Stability) and Fix
# defaults to 100% Roadmap — all editable in-app.
SPLIT_SEED = [
    {"Team": "SWE", "Roadmap": 70, "C&I": 30, "Support": 0,
     "IMS Modernization": 0},
    {"Team": "SWA", "Roadmap": 60, "C&I": 40, "Support": 0,
     "IMS Modernization": 0},
    {"Team": "Cloud", "Roadmap": 50, "C&I": 30, "Support": 20,
     "IMS Modernization": 0},
    {"Team": "QA", "Roadmap": 50, "C&I": 20, "Support": 30,
     "IMS Modernization": 0},
    {"Team": "Load", "Roadmap": 50, "C&I": 25, "Support": 25,
     "IMS Modernization": 0},
    {"Team": "Fix", "Roadmap": 100, "C&I": 0, "Support": 0,
     "IMS Modernization": 0},
]

# Agreed capacities (man-days/month) from the management screenshot, for the
# six H2 months. Editable in-app; other months default to 0.
_AGREED_MONTHS = ["July", "August", "September", "October", "November",
                  "December"]
_AGREED_SEED_VALUES = {
    "Roadmap": [428.5, 439.3, 450.2, 454.8, 454.8, 454.8],
    "C&I": [50.0, 50.0, 50.0, 50.0, 50.0, 50.0],
    "Support": [140.0, 146.2, 150.8, 150.8, 150.8, 150.8],
    "IMS Modernization": [103.9, 103.9, 103.9, 103.9, 103.9, 103.9],
}

# ---- Jira tickets ("reality") and release plan ----------------------------
TICKETS_WS = "Tickets"
RELEASE_WS = "Releases"
LOCAL_TICKETS_FILE = os.path.join(LOCAL_DATA_DIR, "tickets.csv")
LOCAL_RELEASE_FILE = os.path.join(LOCAL_DATA_DIR, "releases.csv")

TICKET_COLUMNS = ["Ticket", "Summary", "Status", "CSA Estimate", "Customer",
                  "Adhoc or RM", "GO live blocker", "Release"]
RELEASE_COLUMNS = ["Release", "CSA Req End", "SWA Req End", "Feature Dev End",
                   "QA Test End", "Internal Release", "External Release",
                   "Status"]

ADHOC_RM_OPTIONS = ["RM", "Adhoc"]        # RM = Roadmap
RELEASE_STATUSES = ["Planned", "In Progress", "Released", "At Risk"]

# A few example tickets so the format and roll-ups are visible; delete/replace
# (or import your RE sheet). CSA Estimate is the effort in working days.
TICKET_SEED = [
    {"Ticket": "RE-101", "Summary": "New billing API", "Status": "In Progress",
     "CSA Estimate": 12, "Customer": "Acme", "Adhoc or RM": "RM",
     "GO live blocker": True, "Release": "2026.08 – Billing GA"},
    {"Ticket": "RE-058", "Summary": "Tenant migration tooling",
     "Status": "To Do", "CSA Estimate": 8, "Customer": "Globex",
     "Adhoc or RM": "RM", "GO live blocker": False,
     "Release": "2026.09 – Platform 2.0"},
    {"Ticket": "RE-142", "Summary": "Urgent prod hotfix", "Status": "Done",
     "CSA Estimate": 3, "Customer": "Acme", "Adhoc or RM": "Adhoc",
     "GO live blocker": False, "Release": ""},
    {"Ticket": "RE-004", "Summary": "Legacy IMS decommission",
     "Status": "To Do", "CSA Estimate": 15, "Customer": "Internal",
     "Adhoc or RM": "RM", "GO live blocker": True,
     "Release": "2026.09 – Platform 2.0"},
]
RELEASE_SEED = [
    # Named to match the current ticket Release tags ("4.x.x"/"3.x.x") so the
    # capacity check attributes effort out of the box. Dates for 4.x.x use the
    # "including adhocs 4.0.0" column; 4.1.0 uses the 4.1.0 column. Edit freely.
    {"Release": "4.x.x", "CSA Req End": "2026-06-19", "SWA Req End": "2026-07-03",
     "Feature Dev End": "2026-07-31", "QA Test End": "2026-08-17",
     "Internal Release": "2026-09-07", "External Release": "2026-09-21",
     "Status": "In Progress"},
    {"Release": "4.1.0", "CSA Req End": "2026-07-31", "SWA Req End": "2026-08-14",
     "Feature Dev End": "2026-09-11", "QA Test End": "2026-09-28",
     "Internal Release": "2026-10-19", "External Release": "2026-11-02",
     "Status": "Planned"},
    {"Release": "3.x.x", "CSA Req End": "", "SWA Req End": "",
     "Feature Dev End": "", "QA Test End": "", "Internal Release": "",
     "External Release": "", "Status": "In Progress"},
]
...
# ---------------------------------------------------------------------------
# Project allocation: per-team split (%) and agreed capacities
# ---------------------------------------------------------------------------

def _coerce_split_df(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy()
    for col in SPLIT_COLUMNS:
        if col not in df.columns:
            df[col] = "" if col == "Team" else 0.0
    df = df[SPLIT_COLUMNS]
...
def _coerce_agreed_df(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy()
    for col in AGREED_COLUMNS:
        if col not in df.columns:
            df[col] = "" if col in ("Category", "Month") else 0.0
    df = df[AGREED_COLUMNS]
    df = df[df["Category"].astype(str).str.strip() != ""]
    for c in ("Category", "Month"):
        df[c] = df[c].astype(str).str.strip()
    df["Agreed"] = pd.to_numeric(df["Agreed"], errors="coerce").fillna(0.0)
    return df.reset_index(drop=True)


def _seed_split_df() -> pd.DataFrame:
    return _coerce_split_df(pd.DataFrame(SPLIT_SEED))


def _seed_agreed_df() -> pd.DataFrame:
    rows = []
    for cat, vals in _AGREED_SEED_VALUES.items():
        for m, v in zip(_AGREED_MONTHS, vals):
            rows.append({"Category": cat, "Month": m, "Agreed": v})
    return _coerce_agreed_df(pd.DataFrame(rows))
...
def load_split_df() -> pd.DataFrame:
    ...
    if conn is not None:
        try:
            return _coerce_split_df(conn.read(worksheet=SPLIT_WS, ttl=0)
                                    .dropna(how="all"))
        except Exception:
            pass
    local = _local_load(LOCAL_SPLIT_FILE)
    return _coerce_split_df(local) if local is not None else _seed_split_df()


def load_agreed_df() -> pd.DataFrame:
    conn = _get_connection()
    if conn is not None:
        try:
            return _coerce_agreed_df(conn.read(worksheet=AGREED_WS, ttl=0)
                                     .dropna(how="all"))
        except Exception:
            pass
    local = _local_load(LOCAL_AGREED_FILE)
    return _coerce_agreed_df(local) if local is not None else _seed_agreed_df()


def save_split_df(df: pd.DataFrame) -> None:
    df = _coerce_split_df(df)
    conn = _get_connection()
    if conn is not None:
        try:
            conn.update(worksheet=SPLIT_WS, data=df)
        except Exception:
            pass
    _local_save(df, LOCAL_SPLIT_FILE)


def save_agreed_df(df: pd.DataFrame) -> None:
    df = _coerce_agreed_df(df)
    conn = _get_connection()
    if conn is not None:
        try:
            conn.update(worksheet=AGREED_WS, data=df)
        except Exception:
            pass
    _local_save(df, LOCAL_AGREED_FILE)


def project_allocation(matrix: pd.DataFrame, split_df: pd.DataFrame,
                       month_labels) -> pd.DataFrame:
    """Split each team's available man-days/month across projects.

    Returns a tidy frame: columns Team, Project, <each month label>. Teams not
    present in ``split_df`` default to 100% Roadmap so totals reconcile.
    """
    if matrix.empty or not month_labels:
        return pd.DataFrame(columns=["Team", "Project"] + list(month_labels))
    team_avail = matrix.groupby("Team")[month_labels].sum()
    split = {r["Team"]: r for _, r in split_df.iterrows()}
    rows = []
    for team, avail in team_avail.iterrows():
        s = split.get(team)
        for proj in PROJECTS:
            pct = (float(s[proj]) if s is not None
                   else (100.0 if proj == "Roadmap" else 0.0)) / 100.0
            rec = {"Team": team, "Project": proj}
            for m in month_labels:
                rec[m] = round(float(avail[m]) * pct, 1)
            rows.append(rec)
    return pd.DataFrame(rows, columns=["Team", "Project"] + list(month_labels))
...
def _coerce_tickets_df(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy()
    for col in TICKET_COLUMNS:
        if col not in df.columns:
            df[col] = (0.0 if col == "CSA Estimate"
                       else False if col == "GO live blocker" else "")
    df = df[TICKET_COLUMNS]
    df["CSA Estimate"] = pd.to_numeric(df["CSA Estimate"],
                                       errors="coerce").fillna(0.0)
    # GO live blocker -> boolean (accepts True/"yes"/"1"/"y").
    df["GO live blocker"] = (df["GO live blocker"].astype(str).str.strip()
                             .str.lower().isin(["true", "yes", "1", "y", "x"]))
    for c in ("Ticket", "Summary", "Status", "Customer", "Adhoc or RM",
              "Release"):
        df[c] = df[c].astype(str).replace({"nan": "", "None": ""}).str.strip()
    df = df[(df["Ticket"] != "") | (df["Summary"] != "")
            | (df["CSA Estimate"] > 0)]
    return df.reset_index(drop=True)


def _coerce_release_df(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy()
...
def _seed_tickets_df() -> pd.DataFrame:
    seed_path = os.path.join(os.path.dirname(__file__), "tickets_seed.json")
    try:
        with open(seed_path, "r", encoding="utf-8") as fh:
            return _coerce_tickets_df(pd.DataFrame(json.load(fh)))
    except Exception:
        return _coerce_tickets_df(pd.DataFrame(TICKET_SEED))


def _rm_or_adhoc(value: str) -> str:
    """Normalise the 'Adhoc or RM' field to 'RM' / 'Adhoc' / '(blank)'."""
    s = str(value).strip().lower()
    if s == "":
        return "(blank)"
    if "adhoc" in s or "ad hoc" in s:
        return "Adhoc"
    if "rm" in s or "roadmap" in s:
        return "RM"
    return value.strip()


def _seed_release_df() -> pd.DataFrame:
    return _coerce_release_df(pd.DataFrame(RELEASE_SEED))
...
def load_tickets_df() -> pd.DataFrame:
    ...
    if conn is not None:
        try:
            return _coerce_tickets_df(conn.read(worksheet=TICKETS_WS, ttl=0)
                                      .dropna(how="all"))
        except Exception:
            pass
    local = _local_load(LOCAL_TICKETS_FILE)
    return _coerce_tickets_df(local) if local is not None else _seed_tickets_df()


def load_release_df() -> pd.DataFrame:
    conn = _get_connection()
    if conn is not None:
        try:
            return _coerce_release_df(conn.read(worksheet=RELEASE_WS, ttl=0)
                                      .dropna(how="all"))
        except Exception:
            pass
    local = _local_load(LOCAL_RELEASE_FILE)
    return _coerce_release_df(local) if local is not None else _seed_release_df()


def save_tickets_df(df: pd.DataFrame) -> None:
    df = _coerce_tickets_df(df)
    conn = _get_connection()
    if conn is not None:
        try:
            conn.update(worksheet=TICKETS_WS, data=df)
        except Exception:
            pass
    _local_save(df, LOCAL_TICKETS_FILE)


def save_release_df(df: pd.DataFrame) -> None:
    df = _coerce_release_df(df)
    conn = _get_connection()
    if conn is not None:
        try:
            conn.update(worksheet=RELEASE_WS, data=df)
        except Exception:
            pass
    _local_save(df, LOCAL_RELEASE_FILE)


def ticket_days_by(tickets: pd.DataFrame, dim: str) -> pd.Series:
    """Sum CSA Estimate (working days) grouped by a ticket dimension."""
    if tickets.empty or dim not in tickets.columns:
        return pd.Series(dtype=float)
    return (tickets.groupby(tickets[dim].replace("", "(blank)"))
            ["CSA Estimate"].sum().sort_values(ascending=False))
...
def _parse_date_eu(value) -> Optional[dt.date]:
    """Parse dates. Slash form (DD/MM/YYYY, as in the milestone sheet) is read
    day-first; ISO (YYYY-MM-DD) and others are parsed normally."""
...
def window_months(start: Optional[dt.date], end: Optional[dt.date],
                  month_labels, year: int) -> list:
    """Month labels whose (year, month) falls within [start, end] inclusive."""
    if start is None or end is None:
        return []
    lo = start.year * 12 + start.month
    hi = end.year * 12 + end.month
    out = []
    for lbl in month_labels:
        num = MONTH_NUMS.get(lbl)
        if num is None:
            continue
        if lo <= year * 12 + num <= hi:
            out.append(lbl)
    return out
...
st.caption("Working-day capacity forecast — aligned to the Replan model in "
           "Teams.xlsx. Months follow the selected forecast year.")
...
with st.sidebar:
    ...
    # Drive the visible months from the chosen year: remaining months of the
    # current year, or the full Jan–Dec for a future year.
    MONTHS = months_for_year(int(forecast_year))
    MONTH_LABELS = [m[1] for m in MONTHS]
    MONTH_NUMS = {m[1]: m[2] for m in MONTHS}
    if MONTH_LABELS:
        st.caption(f"Showing {MONTH_LABELS[0]}–{MONTH_LABELS[-1]} "
                   f"{int(forecast_year)} ({len(MONTH_LABELS)} months)")
...
base_split = load_split_df()
base_agreed = load_agreed_df()
base_tickets = load_tickets_df()
base_release = load_release_df()
...
tab_team, tab_hol, tab_proj, tab_tickets, tab_forecast = st.tabs(
    ["👥 Team & Capacity", "🏖️ Holidays & Absence", "📐 Projects & Variance",
     "🎫 Tickets & Delivery", "📈 Forecast & Export"])
...
# =========================================================================
# TAB 3 — Projects & Variance (project split vs agreed capacities)
# =========================================================================
with tab_proj:
    st.subheader("Project split per team")
    st.caption("How each team's available capacity divides across Roadmap, "
               "C&I (Customization & Integration) and Support. Percentages "
               "should total 100% per team.")

    split_edit = st.data_editor(
        base_split, num_rows="dynamic", use_container_width=True,
        key="split_editor",
        column_config={
            "Team": st.column_config.TextColumn("Team"),
            "Roadmap": st.column_config.NumberColumn(
                "Roadmap %", min_value=0, max_value=100, step=5, format="%d%%"),
            "C&I": st.column_config.NumberColumn(
                "C&I %", min_value=0, max_value=100, step=5, format="%d%%"),
            "Support": st.column_config.NumberColumn(
                "Support %", min_value=0, max_value=100, step=5, format="%d%%"),
            "IMS Modernization": st.column_config.NumberColumn(
                "IMS Modernization %", min_value=0, max_value=100, step=5,
                format="%d%%"),
        })
    split_df = _coerce_split_df(split_edit)
    # Flag any team whose split does not total 100% across all projects.
    bad = [r["Team"] for _, r in split_df.iterrows()
           if abs(sum(r[p] for p in PROJECTS) - 100) > 0.01]
    if bad:
        st.warning("These teams don't total 100%: " + ", ".join(bad))
    if st.button("💾 Save split", type="primary"):
        save_split_df(split_df)
        st.success("Project split saved.")
        st.rerun()

    # ---- Derived (actual) allocation from the live forecast ----
    alloc = project_allocation(matrix, split_df, MONTH_LABELS)
    if alloc.empty:
        st.info("Add team members on the Team & Capacity tab to see allocation.")
    else:
        st.subheader("Capacity by project (man-days / month)")
        actual = alloc.groupby("Project")[MONTH_LABELS].sum().reindex(PROJECTS)
        st.dataframe(
            actual, use_container_width=True,
            column_config={m: st.column_config.NumberColumn(m, format="%.1f")
                           for m in MONTH_LABELS})
        with st.expander("Per-team breakdown"):
            st.dataframe(alloc, use_container_width=True, hide_index=True,
                         column_config={m: st.column_config.NumberColumn(
                             m, format="%.1f") for m in MONTH_LABELS})

        # ---- Agreed capacities (editable, seeded from management) ----
        st.subheader("Agreed capacities (with upper management)")
        st.caption("Targets agreed with management. Edit values, then Save.")
        agreed_wide = (base_agreed.pivot_table(
            index="Category", columns="Month", values="Agreed",
            aggfunc="sum").reindex(index=PROJECTS, columns=MONTH_LABELS)
            .fillna(0.0))
        agreed_edit = st.data_editor(
            agreed_wide, use_container_width=True, key="agreed_editor",
            column_config={m: st.column_config.NumberColumn(m, format="%.1f")
                           for m in MONTH_LABELS})
        if st.button("💾 Save agreed", type="primary"):
            long = agreed_edit.reset_index().melt(
                id_vars="Category", var_name="Month", value_name="Agreed")
            save_agreed_df(long)
            st.success("Agreed capacities saved.")
            st.rerun()

        # ---- Variance: actual (derived) vs agreed ----
        st.subheader("Variance — actual vs agreed")
        st.caption("Δ = actual capacity (from the split) minus the agreed "
                   "target. Positive = surplus over plan, negative = shortfall.")
        agreed_now = agreed_edit.reindex(index=PROJECTS,
                                         columns=MONTH_LABELS).fillna(0.0)
        for proj in PROJECTS:
            a = actual.loc[proj] if proj in actual.index else pd.Series(
                0.0, index=MONTH_LABELS)
            g = agreed_now.loc[proj] if proj in agreed_now.index else pd.Series(
                0.0, index=MONTH_LABELS)
            comp = pd.DataFrame({"Actual": a, "Agreed": g, "Δ": a - g}).T
            st.markdown(f"**{proj}**")
            st.dataframe(
                comp, use_container_width=True,
                column_config={m: st.column_config.NumberColumn(m, format="%.1f")
                               for m in MONTH_LABELS})

        # ---- Chart: actual vs agreed totals across the period ----
        st.subheader("Actual vs agreed (period total)")
        summary = pd.DataFrame({
            "Actual": actual.sum(axis=1),
            "Agreed": agreed_now.sum(axis=1),
        }).reindex(PROJECTS)
        st.bar_chart(summary, color=[BRAND_BLUE, BRAND_ORANGE])

# =========================================================================
# TAB 4 — Tickets & Delivery (Jira "reality" vs allocated capacity)
# =========================================================================
with tab_tickets:
    st.subheader("Jira tickets (reality)")
    st.caption("Delivery work from the RE sheet. CSA Estimate is the effort in "
               "working days; 'Adhoc or RM' marks roadmap vs ad-hoc work; "
               "Go-live blockers are flagged. Edit inline and Save.")

    tickets_edit = st.data_editor(
        base_tickets, num_rows="dynamic", use_container_width=True,
        key="tickets_editor",
        column_config={
            "Ticket": st.column_config.TextColumn("Ticket", help="e.g. PM-3843"),
            "Summary": st.column_config.TextColumn("Summary", width="large"),
            "Status": st.column_config.TextColumn("Status"),
            "CSA Estimate": st.column_config.NumberColumn(
                "CSA Estimate (WD)", min_value=0.0, step=0.5, format="%.1f"),
            "Customer": st.column_config.TextColumn("Customer"),
            "Adhoc or RM": st.column_config.TextColumn("Adhoc or RM"),
            "GO live blocker": st.column_config.CheckboxColumn("Go-live blocker"),
            "Release": st.column_config.TextColumn("Release"),
        })
    tickets_df = _coerce_tickets_df(tickets_edit)
    if st.button("💾 Save tickets", type="primary"):
        save_tickets_df(tickets_df)
        st.success("Tickets saved.")
        st.rerun()

    total_wd = float(tickets_df["CSA Estimate"].sum())
    blockers = tickets_df[tickets_df["GO live blocker"]]
    k1, k2, k3, k4 = st.columns(4)
    k1.metric("Tickets", len(tickets_df))
    k2.metric("Total CSA estimate (WD)", f"{total_wd:,.1f}")
    k3.metric("Go-live blockers", len(blockers))
    k4.metric("Blocker WD", f"{blockers['CSA Estimate'].sum():,.1f}")

    # ---- Go-live blocker risk callout ----
    if not blockers.empty:
        st.error(f"⚠️ {len(blockers)} go-live blockers totalling "
                 f"{blockers['CSA Estimate'].sum():,.1f} working days.")
        with st.expander("Go-live blockers", expanded=True):
            st.dataframe(
                blockers[["Ticket", "Summary", "Status", "CSA Estimate",
                          "Customer", "Release"]],
                use_container_width=True, hide_index=True,
                column_config={"CSA Estimate": st.column_config.NumberColumn(
                    "CSA (WD)", format="%.1f")})

    # ---- RM vs Adhoc: reality against Roadmap allocated capacity ----
    st.subheader("Roadmap vs Adhoc (reality)")
    st.caption("CSA-estimate working days split by RM (roadmap) vs Adhoc. RM "
               "reality is compared with the Roadmap capacity allocated via "
               "the team split (period total).")
    rm_group = tickets_df.assign(RMAdhoc=tickets_df["Adhoc or RM"].map(_rm_or_adhoc))
    by_rm = rm_group.groupby("RMAdhoc")["CSA Estimate"].sum()
    alloc_t = project_allocation(matrix, split_df, MONTH_LABELS)
    roadmap_alloc = (float(alloc_t[alloc_t["Project"] == "Roadmap"]
                     [MONTH_LABELS].sum().sum()) if not alloc_t.empty else 0.0)
    c1, c2, c3 = st.columns(3)
    c1.metric("RM reality (WD)", f"{by_rm.get('RM', 0.0):,.1f}")
    c2.metric("Adhoc reality (WD)", f"{by_rm.get('Adhoc', 0.0):,.1f}")
    c3.metric("Roadmap allocated (WD)", f"{roadmap_alloc:,.1f}")
    st.bar_chart(by_rm.rename("CSA WD"), color=BRAND_ORANGE, horizontal=True)

    # ---- Reality by customer (client budget vs reality seed) ----
    st.subheader("Delivered days by customer (reality)")
    st.caption("Reality side of 'client budget vs reality'. RM and Adhoc shown "
               "separately; add agreed client budgets to complete the compare.")
    if not tickets_df.empty:
        cust = rm_group.copy()
        cust["RMAdhoc"] = cust["RMAdhoc"].where(
            cust["RMAdhoc"].isin(["RM", "Adhoc"]), "Other")
        by_cust = cust.pivot_table(
            index="Customer", columns="RMAdhoc", values="CSA Estimate",
            aggfunc="sum", fill_value=0.0)
        by_cust["Total"] = by_cust.sum(axis=1)
        by_cust = by_cust.sort_values("Total", ascending=False)
        st.dataframe(by_cust, use_container_width=True)

    # ---- Effort by release & by status ----
    cA, cB = st.columns(2)
    with cA:
        st.subheader("By release")
        st.dataframe(ticket_days_by(tickets_df, "Release").rename("CSA WD")
                     .to_frame(), use_container_width=True)
    with cB:
        st.subheader("By status")
        st.dataframe(ticket_days_by(tickets_df, "Status").rename("CSA WD")
                     .to_frame(), use_container_width=True)

    # ---- Release plan (milestone timeline) ----
    st.divider()
    st.subheader("Release plan — milestone timeline")
    st.caption("Key dates per release (ISO YYYY-MM-DD). The feature-development "
               "window (SWA Requirements End → Feature Development End) drives "
               "the capacity check below.")
    _datecol = lambda lbl: st.column_config.TextColumn(lbl, width="small")
    release_edit = st.data_editor(
        base_release, num_rows="dynamic", use_container_width=True,
        key="release_editor",
        column_config={
            "Release": st.column_config.TextColumn("Release"),
            "CSA Req End": _datecol("CSA req end"),
            "SWA Req End": _datecol("SWA req end"),
            "Feature Dev End": _datecol("Feature dev end"),
            "QA Test End": _datecol("QA test end"),
            "Internal Release": _datecol("Internal release"),
            "External Release": _datecol("External release"),
            "Status": st.column_config.SelectboxColumn(
                "Status", options=RELEASE_STATUSES),
        })
    release_df = _coerce_release_df(release_edit)
    if st.button("💾 Save releases", type="primary"):
        save_release_df(release_df)
        st.success("Release plan saved.")
        st.rerun()

    # ---- Release-driven capacity check ----
    st.subheader("Release capacity check")
    st.caption("Required = CSA working-days of tickets tagged to the release. "
               "Roadmap capacity = capacity allocated to Roadmap across the "
               "development-window months. Utilisation = required ÷ capacity.")
    roadmap_by_month = (
        alloc_t[alloc_t["Project"] == "Roadmap"][MONTH_LABELS].sum()
        if not alloc_t.empty else pd.Series(0.0, index=MONTH_LABELS))
    rel_required = tickets_df.groupby("Release")["CSA Estimate"].sum()
    check_rows = []
    for _, r in release_df.iterrows():
        name = r["Release"]
        start = _parse_date_eu(r["SWA Req End"])
        end = _parse_date_eu(r["Feature Dev End"])
        wmonths = window_months(start, end, MONTH_LABELS, int(forecast_year))
        avail = float(sum(roadmap_by_month.get(m, 0.0) for m in wmonths))
        req = float(rel_required.get(name, 0.0))
        util = (req / avail) if avail > 0 else None
        if start is None or end is None:
            flag = "⚪ No dates"
        elif not wmonths:
            flag = "⚪ Window outside horizon"
        elif util is None:
            flag = "⚪ No roadmap capacity"
        elif util > 1.0:
            flag = "🔴 Over"
        elif util >= 0.85:
            flag = "🟠 Tight"
        ...
        check_rows.append({
            "Release": name,
            "Dev window": (f"{start:%d %b} → {end:%d %b}"
                           if start and end else "—"),
            "Months": ", ".join(wmonths) if wmonths else "—",
            "Required WD": round(req, 1),
            "Roadmap capacity WD": round(avail, 1),
            "Utilisation": (f"{util*100:,.0f}%" if util is not None else "—"),
            "Check": flag,
        })
    check_df = pd.DataFrame(check_rows)
    st.dataframe(check_df, use_container_width=True, hide_index=True)

    over = [c["Release"] for c in check_rows if c["Check"].endswith("Over")]
    tight = [c["Release"] for c in check_rows if c["Check"].endswith("Tight")]
    if over:
        st.error("🔴 Over capacity: " + ", ".join(over)
                 + " — required roadmap effort exceeds the capacity available "
                   "in the development window.")
    if tight:
        st.warning("🟠 Tight: " + ", ".join(tight)
                   + " — little slack in the development window.")
...
with dashboard_area:
    ...
    k1, k2, k3, k4 = st.columns(4)
    k1.metric("Total available days", f"{total_days:,.1f}")
    ...
    if not matrix.empty:
        ...
        rng = f"{MONTH_LABELS[0]} → {MONTH_LABELS[-1]}" if MONTH_LABELS else ""
        st.markdown(f"##### Monthly breakdown ({rng})")
        # Render up to 6 month tiles per row so a full 12-month year stays tidy.
        per_row = 6
        prev_total = None
        for start in range(0, len(MONTH_LABELS), per_row):
            chunk = MONTH_LABELS[start:start + per_row]
            ...
            for j, label in enumerate(chunk):
                ...
                cols[j].metric(label, f"{tot:,.1f} d", delta=delta)
                cols[j].caption(f"👥 {ppl} people · ⌀ {avg:.1f} d/person")
                prev_total = tot
    ...
    if matrix.empty:
        ...
        if not alerts:
            st.success(f"No teams drop more than {anomaly_threshold * 100:.0f}% "
                       "month-over-month. Capacity looks stable.")
        else:
...
with tab_forecast:
    ...
    if matrix.empty:
        ...
        st.download_button(
            ...
            data=xlsx_bytes,
            file_name=f"Replan_{int(forecast_year)}.xlsx",
