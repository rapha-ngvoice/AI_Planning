...
APP_VERSION = "v7 · HR leave integration · 2026-06-25"
...
LEAVE_WS = "Leave"        # worksheet holding HR vacation balances
...
LOCAL_LEAVE_FILE = os.path.join(LOCAL_DATA_DIR, "leave.csv")

LEAVE_COLUMNS = ["Name", "HRTeam", "Entitlement", "Unused", "Carryover"]
...
# ---------------------------------------------------------------------------
# HR vacation / leave data (balances that must be taken before year-end)
# ---------------------------------------------------------------------------

def _empty_leave_df() -> pd.DataFrame:
    return pd.DataFrame(columns=LEAVE_COLUMNS)


def _coerce_leave_df(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy()
    for col in LEAVE_COLUMNS:
        if col not in df.columns:
            df[col] = "" if col in ("Name", "HRTeam") else 0.0
    df = df[LEAVE_COLUMNS]
    df = df[df["Name"].astype(str).str.strip() != ""]
    for c in ("Entitlement", "Unused", "Carryover"):
        df[c] = pd.to_numeric(df[c], errors="coerce").fillna(0.0)
    for c in ("Name", "HRTeam"):
        df[c] = df[c].astype(str).replace({"nan": "", "None": ""}).str.strip()
    return df.reset_index(drop=True)


def _seed_leave_df() -> pd.DataFrame:
    seed_path = os.path.join(os.path.dirname(__file__), "leave_seed.json")
    try:
        with open(seed_path, "r", encoding="utf-8") as fh:
            return _coerce_leave_df(pd.DataFrame(json.load(fh)))
    except Exception:
        return _empty_leave_df()


def load_leave_df() -> pd.DataFrame:
    """Read leave balances. Priority: Google Sheets -> local file -> seed."""
    conn = _get_connection()
    if conn is not None:
        try:
            df = conn.read(worksheet=LEAVE_WS, ttl=0)
            return _coerce_leave_df(df.dropna(how="all"))
        except Exception:
            pass
    local = _local_load(LOCAL_LEAVE_FILE)
    if local is not None:
        return _coerce_leave_df(local)
    return _seed_leave_df()


def save_leave_df(df: pd.DataFrame) -> None:
    df = _coerce_leave_df(df)
    conn = _get_connection()
    if conn is not None:
        try:
            conn.update(worksheet=LEAVE_WS, data=df)
        except Exception as exc:
            st.warning(f"Could not write leave to Google Sheets ({exc}). "
                       "Saved to the local file instead.")
    _local_save(df, LOCAL_LEAVE_FILE)
    st.session_state["leave_df"] = df


def _norm_name(s) -> str:
    """Normalise a person name for matching (lowercase, alphanumeric only)."""
    return re.sub(r"[^a-z0-9]+", " ", str(s).lower()).strip()


def build_name_matcher(roster_names):
    """Return a function mapping an HR name to a roster Name (or None).

    Matches on the normalised full name first, then on (first token, last
    token) to catch middle-name / punctuation differences such as
    "Abin Ninan Thomas" vs "Abin Thomas".
    """
    exact, tok = {}, {}
    for rn in roster_names:
        ...
        parts = n.split()
        if len(parts) >= 2:
            tok.setdefault((parts[0], parts[-1]), rn)

    def match(hr_name):
        n = _norm_name(hr_name)
        if n in exact:
            return exact[n]
        parts = n.split()
        if len(parts) >= 2 and (parts[0], parts[-1]) in tok:
            return tok[(parts[0], parts[-1])]
        return None

    return match


def project_leave(leave_df: pd.DataFrame, roster_names, month_labels,
                  mode: str) -> dict:
    """Spread each matched person's unused leave across the forecast months.

    Returns {(roster_name, month_label): projected_leave_days}. ``mode`` is
    either "Spread evenly" or "Weighted to year-end" (later months heavier).
    """
    out: dict = {}
    if leave_df.empty or not month_labels:
        return out
    match = build_name_matcher(roster_names)
    n = len(month_labels)
    weights = [i + 1 for i in range(n)] if mode == "Weighted to year-end" \
        else [1] * n
    wsum = float(sum(weights)) or 1.0
    for _, row in leave_df.iterrows():
        unused = float(row.get("Unused", 0) or 0)
        if unused <= 0:
            continue
        rname = match(row["Name"])
        if not rname:
            continue
        for lbl, w in zip(month_labels, weights):
            out[(rname, lbl)] = out.get((rname, lbl), 0.0) + unused * w / wsum
    return out
...
def build_matrix(team: pd.DataFrame, holidays: pd.DataFrame, year: int,
                 ...
                 admin_sc: float, projected_leave: Optional[dict] = None
                 ) -> pd.DataFrame:
    """Return the full forecast matrix: one row per person, columns per month.

    ``projected_leave`` is an optional {(Name, month_label): days} mapping of
    HR leave balances spread across the months; it is deducted on top of any
    manually logged absences.
    """
    absences = absence_lookup(holidays)
    projected_leave = projected_leave or {}
    # A person split across several rows is still one human, so their absence
    # and projected leave are divided across those rows (totals stay correct).
    name_counts = team["Name"].astype(str).str.strip().value_counts().to_dict()
    records = []
    for _, row in team.iterrows():
        name = str(row["Name"]).strip()
        n_rows = max(int(name_counts.get(name, 1)), 1)
        rec = {
        ...
        for _, label, num in MONTHS:
            logged = (absences.get((name, label), 0.0)
                      + projected_leave.get((name, label), 0.0)) / n_rows
...
with st.sidebar:
    ...
    st.divider()
    st.subheader("🏖️ HR leave")
    project_leave_on = st.toggle(
        "Deduct unused HR leave from forecast", value=False,
        help="Spread each matched person's unused vacation balance across the "
             "forecast months and subtract it from their available capacity.")
    leave_mode = st.radio(
        "Leave distribution", ["Weighted to year-end", "Spread evenly"],
        index=0, disabled=not project_leave_on,
        help="Year-end weighting loads more leave into later months, "
             "reflecting use-it-or-lose-it behaviour.")
...
tab_team, tab_hol, tab_leave, tab_forecast = st.tabs(
    ["👥 Team & Capacity", "🏖️ Holidays & Absence", "🌴 Leave & Risk",
     "📈 Forecast & Export"])
...
# =========================================================================
# TAB 3 — Leave & Risk (HR vacation balances)
# =========================================================================
with tab_leave:
    st.subheader("HR vacation balances")
    st.caption("Unused leave each person still has to take. Turn on "
               "**Deduct unused HR leave from forecast** in the sidebar to "
               "spread these balances across the months as committed absence.")

    edited_leave = st.data_editor(
        base_leave, num_rows="dynamic", use_container_width=True,
        key="leave_editor",
        column_config={
            "HRTeam": st.column_config.TextColumn("HR team"),
            "Entitlement": st.column_config.NumberColumn(
                "Entitlement", format="%.1f"),
            "Unused": st.column_config.NumberColumn(
                "Unused (days)", format="%.1f",
                help="Days still to be taken before year-end."),
            "Carryover": st.column_config.NumberColumn(
                "Carryover", format="%.1f"),
        },
    )
    leave_df = _coerce_leave_df(edited_leave)
    if st.button("💾 Save leave", type="primary"):
        save_leave_df(leave_df)
        st.success("Leave balances saved to the backend.")
        st.rerun()

    # Match status against the roster (so the user can see who gets projected).
    if not leave_df.empty:
        matcher = build_name_matcher(list(team_df["Name"]))
        leave_df = leave_df.copy()
        leave_df["Matched to roster"] = leave_df["Name"].map(
            lambda n: matcher(n) or "—")
        n_matched = int((leave_df["Matched to roster"] != "—").sum())
        st.caption(f"{n_matched} of {len(leave_df)} leave records match a "
                   "roster member and can be projected into the forecast. "
                   "Unmatched people are still shown in the risk view below.")

        # ---- Team rollup (by HR team) ----
        st.subheader("Unused leave by team")
        roll = leave_df.groupby("HRTeam").agg(
            Headcount=("Name", "count"),
            Entitlement=("Entitlement", "sum"),
            Unused=("Unused", "sum")).reset_index()
        roll["% used"] = (1 - roll["Unused"] /
                          roll["Entitlement"].replace(0, pd.NA)) * 100
        roll["Avg unused / head"] = roll["Unused"] / roll["Headcount"]
        roll["Unused = FTE-months"] = roll["Unused"] / max(planable, 1e-9)
        roll = roll.sort_values("Unused", ascending=False)
        st.dataframe(
            roll, use_container_width=True, hide_index=True,
            column_config={
                "Entitlement": st.column_config.NumberColumn(format="%.1f"),
                "Unused": st.column_config.NumberColumn(format="%.1f"),
                "% used": st.column_config.NumberColumn(format="%.0f%%"),
                "Avg unused / head": st.column_config.NumberColumn(format="%.1f"),
                "Unused = FTE-months": st.column_config.NumberColumn(format="%.1f"),
            })

        cL, cR = st.columns([3, 2])
        with cL:
            st.bar_chart(roll.set_index("HRTeam")["Unused"], color=BRAND_ORANGE,
                         horizontal=True)
        with cR:
            tot_unused = float(leave_df["Unused"].sum())
            tot_ent = float(leave_df["Entitlement"].sum())
            st.metric("Total unused leave (days)", f"{tot_unused:,.0f}")
            st.metric("Unused = FTE-months",
                      f"{tot_unused / max(planable, 1e-9):,.1f}")
            st.metric("% of entitlement used",
                      f"{(1 - tot_unused / tot_ent) * 100:,.0f}%"
                      if tot_ent else "—")

        st.subheader("Highest individual balances")
        top = (leave_df.sort_values("Unused", ascending=False)
               .head(12)[["Name", "HRTeam", "Unused", "Carryover",
                          "Matched to roster"]])
        st.dataframe(top, use_container_width=True, hide_index=True,
                     column_config={
                         "Unused": st.column_config.NumberColumn(format="%.1f"),
                         "Carryover": st.column_config.NumberColumn(format="%.1f")})
...
# =========================================================================
# Project HR leave into the forecast months when the sidebar toggle is on.
projected = (project_leave(leave_df, list(team_df["Name"]), MONTH_LABELS,
                           leave_mode)
             if project_leave_on else {})
matrix = build_matrix(team_df, holiday_df, int(forecast_year), max_days,
                      sickness, holiday_allow, admin_sc, projected)
...
with dashboard_area:
    ...
    if project_leave_on and projected:
        leave_days = sum(projected.values())
        st.caption(f"🌴 HR leave projected into the forecast: "
                   f"**{leave_days:,.0f} days** deducted across "
                   f"{len(MONTH_LABELS)} months ({leave_mode.lower()}). "
                   "Turn off in the sidebar to see gross capacity.")
